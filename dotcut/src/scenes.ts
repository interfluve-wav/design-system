...
export function rasterize(
  ...
  if (scene.kind === "checker") {
    ...
    // Text scene: carve the letter glyph directly
    const text = (scene.value || "B").trim();
    if (text) {
      const letterMask = rasterizeText(text, cols, rows, fontFamily);
      for (let i = 0; i < cols * rows; i++) {
        if (letterMask[i] === 0) out[i] = 0;
      }
    }
  }

  // Overlay B/O/N/K letter on top of any scene as additional negative space
  if (letter && letter.trim()) {
    const letterMask = rasterizeText(letter, cols, rows, fontFamily);
    for (let i = 0; i < cols * rows; i++) {
      if (letterMask[i] === 0) out[i] = 0;
    }
  }
