...
export class DotCut {
  ...
  private applyScene(scene: Scene, instant: boolean) {
    const currentLetter = LETTERS[this.letterIdx % LETTERS.length];
    const next = rasterize(scene, this.cols, this.rows, this.fontFamily, currentLetter);
  ...
  advance() {
    ...
    this.letterIdx = (this.letterIdx + 1) % LETTERS.length;
...
function roundedSquare(
  ...
  for (let i = 1; i <= steps; i++) {
    ...
    const y = Math.sign(sv) * Math.pow(Math.abs(sv), 2 / n) * r;
